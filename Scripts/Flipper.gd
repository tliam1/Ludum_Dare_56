...
func _ready():
	flipper_mesh.rotation_degrees.y = (90 if flipperType == FlipperType.LEFT_FLIPPER else -90)
...
func rotate_flipper_to(target_angle: float):
	...
	tween.tween_property(flipper_mesh, "rotation_degrees:y", target_angle, 0.25).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_LINEAR).from_current() 
